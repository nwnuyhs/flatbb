<?php /** New/edit topic. Variables: topic (null for new), post, categories, category_id, tags, action */ ?>
<div class="compose">
  <h1><?= $topic ? t('Edit Topic') : t('New Topic') ?></h1>
  <form method="post" action="<?= h($action) ?>" data-ajax="1" data-composer>
    <?= csrf_field() ?>
    <div class="form-row"><input type="text" name="title" class="input-lg" placeholder="<?= t('Title') ?>" value="<?= h($topic['title'] ?? '') ?>" maxlength="200" required autofocus></div>
    <div class="form-grid">
    ...
    <?= editor('body', (string)($post['body'] ?? ''), t('Write your post…'), ['scope' => $topic ? 'topic-' . (int)$topic['id'] : 'topic-new', 'ctx' => ['topic' => $topic]]) ?>
