...
<?php foreach ($tree[0] ?? [] as $c): $lt = $last_topics[(int)$c['last_topic_id']] ?? null; ?>
  <article class="category-row">
    <div class="cat-main">
      <h3><a href="<?= h(category_url($c)) ?>"><span class="cat-dot" style="background:<?= h($c['color'] ?: '#999') ?>"></span><?= h($c['name']) ?></a></h3>
      <?php if ($c['description']): ?><p class="muted"><?= h($c['description']) ?></p><?php endif; ?>
      <?php if (!empty($tree[(int)$c['id']])): ?><div class="cat-children"><?php foreach ($tree[(int)$c['id']] as $ch): ?><a class="cat-badge" href="<?= h(category_url($ch)) ?>"><span class="cat-dot" style="background:<?= h($ch['color'] ?: '#999') ?>"></span><?= h($ch['name']) ?></a><?php endforeach; ?></div><?php endif; ?>
    </div>
