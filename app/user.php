...
function user_settings(string $tab = 'profile'): never
{
    ...
    if (is_post()) {
        ...
        if ($tab === 'profile') {
            ...
            if (password_check($new) !== '') fail(password_check($new), $back);
    ...
    if ($tab === 'points') {
        ...
        $extra = '<p class="points-balance"><b>' . human_number((int)$me['points']) . '</b> ' . t('points') . ' <a class="btn btn-sm" href="' . h(url('/points')) . '">' . icon('coin') . t('My points page') . '</a> <span class="muted small">' . t('Only you and the staff can see this history.') . '</span></p>'
