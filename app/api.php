...
function seo_sitemap(): never
{
    ...
    $rows = feed_topics(all("SELECT id,slug,category_id,updated_at,meta FROM fb_topics WHERE {$where} ORDER BY id DESC LIMIT 5000"));
...
function seo_rss(): never
{
    ...
    $visible = category_visible_ids();
    $where = 'is_deleted=0' . ($visible !== null ? ($visible === [] ? ' AND 0' : ' AND category_id IN (' . implode(',', $visible) . ')') : '');
    $rows = feed_topics(all("SELECT id,title,slug,category_id,first_post_id,user_id,created_at,meta FROM fb_topics WHERE {$where} ORDER BY id DESC LIMIT 30"));
