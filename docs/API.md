...
Generated by `php flatbb api:list` on 2026-09-07. Every function below is global and available to plugins.
...
- `plugin_publish(string $id, string $token, string $changelog = '', string $endpoint = '', bool $insecure = false, array $images = []): array` — $images: screenshots to publish with the version, each ['path' => local file, 'name' => file name] (up to 5; they replace the plugin's set on the marketplace).
...
- `plugin_enable(string $id): void` — Enable a plugin and record the version that is on disk. In the request that uploaded a new zip the manifest was already loaded (the previous version), so the version and the install routine come from a fresh read of the file with the tokenizer.
...
- `category_icon(array $cat): string` — The category's icon when the admin picked one in Admin → Categories, else ''.
