...
Generated by `php flatbb api:list` on 2026-09-09. Every function below is global and available to plugins.
...
- `layout_core_items(string $region): array` — The items the core itself puts into a list region (label only), so Admin → Layout can order them next to plugin items.
...
- `lang_set(string $code): void` — Remember a visitor's language: cookie for a year, and the preference of a signed-in member. '' returns to the site default.
...
- `header_right_items(?array $me, int $unread, array $user_menu): array` — The right side of the header as the list region header.right: search, new topic, language, theme, notifications and the account menu (sign in / sign up for guests). Plugins add items (['html' => …, 'label' => …, 'weight' => …]) and Admin → Layout hides or reorders everything. Weights: search -20, new topic -10, plugin items 0 unless they say otherwise (so a new icon lands right after the new-topic button), language 10, theme 20, notifications 30, account 40. The two older HTML regions header.right.before_search / after_search still work and sit around the search box.
...
- `admin_category_icon_value(int $id, string $posted): string` — The icon value to store: a built-in icon name, or the category's own uploaded image when it is kept; anything else is none.
- `admin_category_icon_unlink(int $id): void` — Remove a category's uploaded icon file(s) (uploads/site/cat_<id>.*).
- `admin_icon_picker(string $value): string` — Built-in icons as real tiles plus the uploaded image (when any) and a file input; a click sets the hidden "icon" field (app.js).

## app/admin_system.php

- `admin_layout_items(string $region): array` — Every item of a list region as the site shows it: the core's own entries plus what plugins add, in the effective order (hidden ones included).
