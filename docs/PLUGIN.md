...
- High-risk admin pages call `need_sudo()` after `need_admin()`: the admin re-enters the password once per ten minutes, so a stolen or script-ridden session cannot change what matters. Log what an admin changed with `admin_log('myid.action', $target, $detail)`; it shows under Tools and fires `admin.action`.
