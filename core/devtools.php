...
function docs_hook_descriptions(): array
{
    return [
        ...
        'topic.category_missing' => 'Filter: a new topic arrived without a category (value 0; ctx: title, body, user). Return a category id to file it there (an AI pick, say); the writer must be allowed to post in it. Runs before the default category.',
        'topic.category_auto' => 'Filter: whether a plugin will pick the category of a new topic (bool). True makes the category optional in the composer; return true only when topic.category_missing can actually answer (configured, within limits).',
        'link.previewable' => 'Filter: whether a link may get a preview card (bool; ctx url, host). Return false to keep a link a link, e.g. when your plugin shows its own player for that site.',
        'ai.request' => 'Filter: a request to the shared AI connection before it is sent (value: system, user, opts; ctx: purpose = the calling plugin). Change it, or return an array with an error key to refuse it (quotas, redaction).',
        'ai.response' => 'Event: after each attempt on an AI connection (ctx: purpose, connection = 1 for the main one, 2 and 3 for the backups, model, usage [in, out] tokens, ok). A request that falls back fires it once per connection tried. For plugins that count or log usage.',
        ...
        'topic.after_save' => 'After a topic was created or edited (ctx: topic_id, post_id, new). A new topic that waits in the review queue fires it when a moderator approves it, so the topic is public by then.',
        'review.hold' => 'Filter: why a new topic or reply waits in the review queue (value: the reason so far, an English text shown through t(), or empty; ctx: kind topic|reply, user, category, text). Return a reason to hold it. Administrators and moderators are never held.',
        'review.held' => 'Event: a topic or reply was put in the review queue (ctx: id, kind, topic_id, post_id, user_id, reason).',
        'review.decided' => 'Event: a moderator approved (status 1) or rejected (status 2) an item of the review queue (ctx: id, status, by). On approval the usual topic.after_save / post.after_save fire as well.',
        ...
        'post.after_save' => 'After a post was created or edited. A new reply that waits in the review queue fires it on approval.',
        'post.after_delete' => 'After a reply was deleted or restored.',
        'post.after_like' => 'After a like toggle.',
        'notification.before_create' => 'Filter/veto a notification (return null to skip). Also runs once per recipient inside notify_many() (ctx: many = true): no database queries.',
        'notification.after_create' => 'After a notification was stored.',
        'notification.after_create_many' => 'After notify_many() stored the same notification for many members (ctx: user_ids, from_user_id, kind, topic_id, post_id).',
        ...
        'points.icon' => 'The icon name of a points history line (ctx: reason, delta). Runs inside lists: no database access.',
